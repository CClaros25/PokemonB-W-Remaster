...
// ===== POKEDEX AND PARTY STATE (persistent) =====
...
// ===== SIDE PANEL STATE =====
let sidePanelMode = "main"; // "main", "battle", "party", "battle_party"
let sidePanelSceneRef = null;
let slotOptionsPanel = null; // For party slot options
  ...
  if (sidePanelSceneRef) {
    if (mode === "main") {
      ...
      renderPartyPanel(sidePanelSceneRef, "battle", (idx) => {
        if (party[idx]) {
          // Switch active battle Pokémon to this
          movePartySlot(idx, 0);
...
function startEncounter(scene) {
  ...
  setSidePanelMode("battle");
  // Hide overworld: trees, rocks, player
